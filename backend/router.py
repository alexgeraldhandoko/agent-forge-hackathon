...
def fallback_train_update(prompt: str, username: str, store: WorkspaceStore) -> dict[str, Any]:
    current = store.files.get("training_job.py", "")
    content = (
        current.rstrip()
        + "\n\n"
        + "def run_training_job():\n"
        + f"    return {{\"status\": \"queued\", \"owner\": \"{username}\", \"prompt\": {prompt[:160]!r}}}\n"
    ).lstrip()
    return {
        "explanation": "Prepared a simulated Nosana GPU training job for the shared workspace.",
        "assistant_message": "I prepared a training job file and queued the Nosana route for this shared workspace.",
        "files": {"training_job.py": content},
        "functions_modified": ["run_training_job"],
        "nosana_script": f"echo {json.dumps('training requested by ' + username)}",
        "provider": "Nosana",
    }
